 ...
 */
public class InteractiveGaugeSkin extends SkinBase<Gauge> implements Skin<Gauge> {
    ...
    private void registerListeners() {
        ...
        getSkinnable().addGaugeObserver(MedusaEvt.FINISHED, e -> handleEvents(e.getEvtType().getName()));
    ...
    protected void handleEvents(final String EVENT_TYPE) {
        if ("RESIZE".equals(EVENT_TYPE)) {
            ...
            if (getSkinnable().isInteractive()) {
                ...
                needle.addEventHandler(MouseEvent.MOUSE_PRESSED, e -> { if (getSkinnable().isDisabled()) return; touchRotate(e.getSceneX(), e.getSceneY()); });
                needle.addEventHandler(MouseEvent.MOUSE_DRAGGED, e -> { if (getSkinnable().isDisabled()) return; touchRotate(e.getSceneX(), e.getSceneY()); });
    ...
    private void touchRotate(final double X, final double Y) {
        Point2D p      = getSkinnable().sceneToLocal(X, Y);
        ...
        getSkinnable().setValue(angle / angleStep + minValue);
    }
